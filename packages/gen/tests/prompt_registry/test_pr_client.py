...
from gen_ai_hub.orchestration_v2.models.message import UserMessage, TextPart, ImagePart
...
from gen_ai_hub.orchestration_v2.models.multimodal_items import ImageItem, ImageUrl


class TestPromptTemplateModelContent(unittest.TestCase):

    def test_text_part(self):
        prompt_template_obj = PromptTemplate(role='system', content=['Hello, world!'])
        self.assertIsInstance(prompt_template_obj.content[0], TextPart)
        self.assertEqual(prompt_template_obj.content[0].text, 'Hello, world!')

    def test_image_part(self):
        prompt_template_obj = PromptTemplate(role='system', content=[ImageItem(url="https://example.com/image.png")])
        self.assertIsInstance(prompt_template_obj.content[0], ImagePart)
        self.assertIsInstance(prompt_template_obj.content[0].image_url, ImageUrl)

    def test_text_and_image_part(self):
        prompt_template_obj = PromptTemplate(role='system', content=['Hello, world!', ImageItem(url="https://example.com/image.png")])
        self.assertIsInstance(prompt_template_obj.content[0], TextPart)
        self.assertIsInstance(prompt_template_obj.content[1], ImagePart)
        self.assertIsInstance(prompt_template_obj.content[1].image_url, ImageUrl)
        self.assertEqual(prompt_template_obj.content[0].text, 'Hello, world!')
        self.assertEqual(prompt_template_obj.content[1].image_url.url, 'https://example.com/image.png')

    def test_invalid_content(self):
        with self.assertRaises(ValueError):
            PromptTemplate(role='system', content=123)

    def test_invalid_content_list(self):
        with self.assertRaises(ValueError):
            PromptTemplate(role='system', content=[123, 'Hello, world!'])
