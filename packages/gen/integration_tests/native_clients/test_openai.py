...
class OpenAITests(TestCaseAICoreSetupMixin, unittest.TestCase):
    ...
    def test_gpt_model_version_in_params(self, model_name=OPENAI_GPT_5_TEST_MODEL_NANO, model_version="latest"):
        messages = self._get_test_messages()
    ...
    def test_chat_completion_global(self, model=OPENAI_GPT_5_TEST_MODEL_NANO):
    ...
    def test_responses(self, model=OPENAI_GPT_5_TEST_MODEL):
        client = OpenAI(proxy_client=self.proxy_client, max_retries=10)
    ...
    def test_responses_streaming(self, model=OPENAI_GPT_5_TEST_MODEL):
        client = OpenAI(proxy_client=self.proxy_client, max_retries=10)
        response = client.responses.create(
            model=model,
            instructions="You are a helpful assistant.",
        ...
        assert isinstance(response, openai.Stream)


    def test_responses_global(self, model=OPENAI_GPT_5_TEST_MODEL):
    ...
    def test_responses_structured_outputs(self, model=OPENAI_GPT_5_TEST_MODEL):
        client = OpenAI(proxy_client=self.proxy_client, max_retries=10)
    ...
    @unittest.skip("Web search tool temporary is not supported.")
    def test_responses_with_web_search_tool(self, model=OPENAI_GPT_5_TEST_MODEL):
        client = OpenAI(proxy_client=self.proxy_client, max_retries=10)
        response = client.responses.create(
            model=model,
            instructions="You are a helpful assistant.",
            input="What is the current weather in Tbilisi?",
...
class AsyncOpenAITests(TestCaseAICoreSetupMixin, unittest.IsolatedAsyncioTestCase):
    ...
    async def test_async_chat_completion_with_model_version_param(self, model=OPENAI_GPT_5_TEST_MODEL_NANO):
        prompt = 'Why is the sky blue?'
    ...
    async def test_async_responses(self, model=OPENAI_GPT_5_TEST_MODEL):
        client = AsyncOpenAI(proxy_client=self.proxy_client, max_retries=10)
    ...
    async def test_async_responses_streaming(self, model=OPENAI_GPT_5_TEST_MODEL):
        client = AsyncOpenAI(proxy_client=self.proxy_client, max_retries=10)
        response = await client.responses.create(
            model=model,
            instructions="You are a helpful assistant.",
        ...
        assert isinstance(response, openai.AsyncStream)

    async def test_async_responses_structured_outputs(self, model=OPENAI_GPT_5_TEST_MODEL):
        client = AsyncOpenAI(proxy_client=self.proxy_client, max_retries=10)
