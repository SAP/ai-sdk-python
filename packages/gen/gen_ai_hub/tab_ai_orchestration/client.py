...
from gen_ai_hub.tab_ai_orchestration.generated.models.predict_response import (
    PredictResponse,
)
...
class _SapRESTClientObject(RESTClientObject):
    def __init__(
        self, configuration: Configuration, proxy: GenAIHubProxyClient, timeout: Any
    ) -> None:
    ...
    def _create_pool_manager(self) -> httpx.AsyncClient:
        kwargs: dict[str, Any] = {
            "event_hooks": {"request": [_make_auth_hook(self._proxy)]}
        }
...
class _SapApiClient(ApiClient):
    ...
    def sanitize_for_serialization(self, obj: Any) -> Any:
        ...
        if isinstance(obj, BaseModel):
            d = obj.model_dump(
                by_alias=True, exclude_none=True, exclude={"additional_properties"}
            )
...
class TabAiOrchestrationClient:
    ...
    Usage::
        ...
        response = await client.predict(predict_request=...)
        await client.close()
    """
    ...
    async def predict(
        ...
        return await self._predict_api.predict(  # type: ignore[no-any-return]
            ai_resource_group=self._resource_group,
