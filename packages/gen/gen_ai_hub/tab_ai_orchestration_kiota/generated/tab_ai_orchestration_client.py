...
if TYPE_CHECKING:
    from .predict.predict_request_builder import PredictRequestBuilder

class TabAiOrchestrationClient(BaseRequestBuilder):
    """
    The main entry point of the SDK, exposes the configuration and the fluent API.
    """
    def __init__(self,request_adapter: RequestAdapter) -> None:
        """
        Instantiates a new TabAiOrchestrationClient and sets the default values.
        ...
        if not self.request_adapter.base_url:
            self.request_adapter.base_url = "/v1"
        self.path_parameters["base_url"] = self.request_adapter.base_url
    
    @property
    def predict(self) -> PredictRequestBuilder:
        """
        The predict property
        """
        from .predict.predict_request_builder import PredictRequestBuilder

        return PredictRequestBuilder(self.request_adapter, self.path_parameters)
