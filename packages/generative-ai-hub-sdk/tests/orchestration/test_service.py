...
from gen_ai_hub.orchestration.service import OrchestrationService, cache_if_not_none
from tests.mock import (
    get_mocked_ai_core_client,
    ai_core_ai_api_mocker,
    orchestration_completion_mocker,
    orchestration_stream_completion_mocker,
    orchestration_stream_completion_mocker_async,
...
class TestOrchestrationService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = OrchestrationConfig(
            llm=LLM(name="gemini-2.5-flash-lite"),
            template=Template(
                messages=[
    ...
    def test_initialization_with_empty_api_url(self):
        with ai_core_ai_api_mocker(auth_url=self.proxy_client.auth_url, base_url=self.proxy_client.base_url):
            ...
            self.assertEqual(client.api_url, "https://api.ai.internalprod.eu-central-1.aws.ml.hana.ondemand.com/v2/inference/deployments/d7f9c215310f5a11")

    def test_initialization_with_config_name(self):
        with ai_core_ai_api_mocker(auth_url=self.proxy_client.auth_url, base_url=self.proxy_client.base_url):
            client = OrchestrationService(config_id="0152d9f0-694f-4bd2-a287-f7d270c9db60", proxy_client=self.proxy_client)
        self.assertEqual(client.api_url, "https://api.ai.internalprod.eu-central-1.aws.ml.hana.ondemand.com/v2/inference/deployments/dea20c27f7fe0eca")
    ...
    def test_run_with_config(self):
        with ai_core_ai_api_mocker(auth_url=self.proxy_client.auth_url, base_url=self.proxy_client.base_url):
            ...
            with orchestration_completion_mocker(client.api_url + '/completion'):
                client.run(config=self.config)

    def test_stream_with_config(self):
        with ai_core_ai_api_mocker(auth_url=self.proxy_client.auth_url, base_url=self.proxy_client.base_url):
            client = OrchestrationService(api_url=self.api_url, proxy_client=self.proxy_client)
            with orchestration_stream_completion_mocker(client.api_url + '/completion'):
                txt = ''
                for chunk in client.stream(config=self.config):
                    if chunk.orchestration_result.choices:
                        txt += chunk.orchestration_result.choices[0].delta.content
                self.assertEqual(txt, 'This confirms receipt of the system message: "Hello, World!\n')

    def test_too_many_requests(self):
        with ai_core_ai_api_mocker(auth_url=self.proxy_client.auth_url, base_url=self.proxy_client.base_url):
            client = OrchestrationService(api_url=self.api_url, proxy_client=self.proxy_client)
            with orchestration_too_many_requests_mocker(client.api_url + '/completion'):
                with self.assertRaises(OrchestrationError) as context:
                    client.run(config=self.config)
                self.assertIn('X-Custom-Header', cast(OrchestrationError, context.exception).http_headers)
        ...
        with ai_core_ai_api_mocker(auth_url=self.proxy_client.auth_url, base_url=self.proxy_client.base_url):
            ...
            with orchestration_too_many_requests_mocker(client.api_url + '/completion'):
                with patch.object(client, 'handle_retry', side_effect=track_retry):
                    ...
                    self.assertIn('X-Custom-Header', error.http_headers)
    ...
    def test_timeout_client_request(self):
        class FakeResponse:
            ...
            def json(self):
                return GET_ORCHESTRATION_COMPLETION_RESPONSE
...
class TestOrchestrationServiceAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_async_stream_with_config(self):
        with ai_core_ai_api_mocker(auth_url=self.proxy_client.auth_url, base_url=self.proxy_client.base_url):
            client = OrchestrationService(api_url=self.api_url, proxy_client=self.proxy_client)
            async with orchestration_stream_completion_mocker_async(client.api_url + '/completion'):
                txt = ''
                async for chunk in await client.astream(config=self.config):
                    if chunk.orchestration_result.choices:
                        txt += chunk.orchestration_result.choices[0].delta.content
                self.assertEqual(txt, 'This confirms receipt of the system message: "Hello, World!\n')
