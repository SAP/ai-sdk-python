...
class TestSSEClientBuffering(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_json_split_across_chunks(self):
        ...
        self.assertEqual(results[0].request_id, "test-request-id")
        self.assertEqual(results[0].intermediate_results.llm.choices[0].delta.content, "split test")
    ...
    async def test_multiple_events_in_single_chunk(self):
        ...
        self.assertEqual(len(results), 2)
        self.assertEqual(results[0].intermediate_results.llm.choices[0].delta.content, "first")
        self.assertEqual(results[1].intermediate_results.llm.choices[0].delta.content, "second")
    ...
    async def test_final_message_stops_iteration(self):
        ...
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].intermediate_results.llm.choices[0].delta.content, "before done")
    ...
    async def test_partial_line_at_end_of_stream(self):
        ...
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].request_id, "test-request-id")
        self.assertEqual(results[0].intermediate_results.llm.choices[0].delta.content, "final")
    ...
    async def test_complex_json_with_nested_objects(self):
        ...
        self.assertEqual(len(results), 1)
        self.assertEqual(results[0].request_id, "test-request-id")
        self.assertEqual(results[0].intermediate_results.llm.choices[0].delta.content, "Hello")

    async def test_error_event_with_list_payload_raises_orchestration_error(self):
        ...
        with self.assertRaises(OrchestrationErrorList) as ctx:
            async for _ in client._internal_iterator():
                self.fail("Iterator must not yield results when error event is received")

        exc = ctx.exception

        self.assertIsInstance(exc.errors, list)
        self.assertEqual(len(exc.errors), 2)
        self.assertEqual(exc.errors[0].code, "E_FIRST")
        self.assertEqual(exc.errors[1].code, "E_SECOND")
